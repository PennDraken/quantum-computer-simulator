...
def add_gate_circuit(gateButtons : [[MenuButton]], gateList : [(str, [int])], x : int, y : int, circuit_dx : int, circuit_dy : int, input_boxes):
    for button in gateButtons:
        if button.selected and (Mouse.r_held or Mouse.r_click):
            ...
            # Here we place the gate on the circuit
            ...
            for i in range(0,len(gateList)):
                if checkLines(Mouse.x, Mouse.y, ((x + circuit_dx) + 50 * i) - 20, ((x + circuit_dx) + 50 * i) + 20):
                    ...
                    check = True
            if not check:  # Place gate at end of circuit
