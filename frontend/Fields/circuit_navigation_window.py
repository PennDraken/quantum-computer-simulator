...
class Circuit_Navigation_Window:
    ...
    def draw(self):
        ...
        box_y = self.height/2-self.button_size/2
        for i in range(0, len(self.options)):
            distance_between_boxes = self.width / len(self.options)
            box_x = distance_between_boxes * i + distance_between_boxes/2 - self.button_size/2
            # Draw button square
            pygame.draw.rect(self.screen, Colors.white, (box_x, box_y, self.button_size, self.button_size), width = 2)
            # Draw button text centered on square
            text(self.screen, self.options[i], box_x + self.button_size/2, box_y + self.button_size/2, Colors.white, self.title_font)
    ...
    def click(self, x, y):
        ...
        for i in range(0, len(self.options)):
            ...
            if x > box_x and x < box_x + self.button_size and y > box_y and y < box_y + self.button_size:
